...
with st.sidebar:
    ...
    # ── DAFTAR MENU (Opsi 2 — klik = buka panel di area utama) ──
    ...
    with menu_list:
        ...
        for key, icon, label_key in [
            ...
            btn_wrap = st.container(key=f"menu_{key}")
            with btn_wrap:
                if st.button(t(label_key), key=f"menu_btn_{key}", use_container_width=True):
                    st.session_state.active_panel = None if st.session_state.active_panel == key else key
                    st.rerun()
...
# =====================
# 10c. PANEL AREA UTAMA (Opsi 2)
# =====================
if st.session_state.get("active_panel"):
    _panel_key = st.session_state.active_panel
    with st.container(border=True):
        head_col1, head_col2 = st.columns([8, 1])
        with head_col1:
            st.markdown(f"#### {t(PANEL_TITLES[_panel_key])}")
        with head_col2:
